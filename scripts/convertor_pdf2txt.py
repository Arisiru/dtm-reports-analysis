# this script requeres pdftotext installed, for mac install it with home brew poppler
            ...
            if '.pdf' in report_pdf:
                try:
                    ...
                    txt_report_path = os.path.join(
                        dir_data_txt, findex, ticker, "%s.txt" % report_name)
                except:
                ...
                if not os.path.exists(txt_report_path):
                    ...
                    command = "pdftotext %s %s" % (
                        pdf_report_path, txt_report_path)
                    os.system(command)
                    if os.path.exists(txt_report_path):
                        result_file = open(txt_report_path, 'r')
                        characters = sum(len(text_line)
                                         for text_line in result_file)
                        print("%s converted, len=%s" %
